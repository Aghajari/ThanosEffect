...
import com.aghajari.thanoseffect.EffectUtils.PARTICLE_BYTES
...
internal class ViewRenderer(
    ...
    // Draws only the first frame of the DustThread
    ...
    private fun isRunning(): Boolean {
        if (endRenderingTime == 0f) {
            ...
            val isRunning =
                time <= maxLifetime + EffectUtils.MAX_DELTA
            if (!isRunning) {
                die()
            }
            return isRunning
    ...
    private fun renderNextLinesIfReady(particles: FloatBuffer) {
        ...
        val delay = EffectUtils.MAX_DELTA.toInt() * 1000
        ...
        while (c < count) {
            ...
            while (y < bounds.bottom) {
                ...
                val lifeTime = particleLifeTime.calculateLifeTime(line, maxLine).toFloat()
                ...
                particles.put(EffectUtils.randomInitialAlpha(color))
